function ajax_data(str_id) {
  ...
  document.cookie = "name=setpoints; {volts:12}";

  var url = `http://perlworks.com:3001/xhr/${str_id}`;

   $.ajax({url: url, 
      success: function(data){ 

         //console.log(data['volts']) 
         ...
         for ( let key in data['volts']){
            ...
            if ( td != null ){
               ...
               if (data['volts'][key] >= 12.5){
                  td.style.backgroundColor  = "red"
               }else if ( data['volts'][key] <= 12.1 ){
...
function str_data(str_id) {

   if ( str_id < 1 || str_id > 4 ) {
      console.error("str_id out of range " + str_id)
      return false;
   }
   ...
   var url = `http://perlworks.com:8086/query?pretty=true&db=bems&q=SELECT+${str_select['volts'][str_id]}+FROM+volts+limit+1`;
